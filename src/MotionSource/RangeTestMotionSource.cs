...
namespace ToySerialController.MotionSource
{
    public class RangeTestMotionSource : IMotionSource
    {
        ...
        public void CreateUI(IUIBuilder builder)
        {
            ...
            AxisChooser = _group.CreateScrollablePopup("MotionSource:Axis", "Select Axis", new List<string> { "X", "Y", "Z", "RX", "RY", "RZ" }, "X", AxisChooserCallback);
        ...
        public bool Update()
        {
            ...
            if (AxisChooser.val == "X") position = _rangeStart + ReferenceUp * Mathf.Lerp(0, ReferenceLength, t);
            if (AxisChooser.val == "Y") position = _rangeStart + ReferenceRight * Mathf.Lerp(-ReferenceLength, ReferenceLength, t);
            if (AxisChooser.val == "Z") position = _rangeStart + ReferenceForward * Mathf.Lerp(-ReferenceLength, ReferenceLength, t);

            ReferencePosition = Vector3.Lerp(ReferencePosition, position, Mathf.Lerp(0, 1, SpeedSlider.val / 10.0f));

            var rotation = Quaternion.identity;
            if (AxisChooser.val == "RX") rotation = Quaternion.Euler(0, Mathf.Lerp(0, 360, t), 0);
            if (AxisChooser.val == "RY") rotation = Quaternion.Euler(Mathf.Lerp(-90, 90, t), 0, 0);
            if (AxisChooser.val == "RZ") rotation = Quaternion.Euler(0, 0, Mathf.Lerp(-90, 90, t));
