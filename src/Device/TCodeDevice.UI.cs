...
namespace ToySerialController
{
    public partial class TCodeDevice : IDevice
    {
        ...
        private void CreateL0AxisUI(IUIBuilder builder)
        {
            ...
            RangeMaxL0Slider = group.CreateSlider("Device:L0:RangeMax", "Range Max (%)", 1f, 0.01f, 1f, v => 
            {
                if(RangeMinL0Slider.val >= v)
                {
                    RangeMinL0Slider.val = v - 0.01f;
                }
            }, true, true, true, "P0");
            RangeMinL0Slider = group.CreateSlider("Device:L0:RangeMin", "Range Min (%)", 0f, 0f, 0.99f, v => 
            {
                if(RangeMaxL0Slider.val <= v)
                {
                    RangeMaxL0Slider.val = v + 0.01f;
                }
            }, true, true, true, "P0");
            OutputMaxL0Slider = group.CreateSlider("Device:L0:OutputMax", "Output Max (%)", 1f, 0.01f, 1f, v => 
            {
                if(OutputMinL0Slider.val >= v)
                {
                    OutputMinL0Slider.val = v - 0.01f;
                }
            }, true, true, true, "P0");
            OutputMinL0Slider = group.CreateSlider("Device:L0:OutputMin", "Output Min (%)", 0f, 0f, 0.99f, v => 
            {
                if(OutputMaxL0Slider.val <= v)
                {
                    OutputMaxL0Slider.val = v + 0.01f;
                }
            }, true, true, true, "P0");
